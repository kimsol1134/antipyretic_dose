...
export const faqData: FAQItem[] = [
  {
    ...
    detailedAnswer: `
      <h4 class="font-semibold text-gray-800 mb-2">해열제를 고려해야 하는 경우</h4>
      ...
      <h4 class="font-semibold text-gray-800 mb-2">특별 주의 (즉시 병원 방문)</h4>
      <ul class="list-disc pl-5 space-y-1 mb-4">
        <li class="text-red-600 font-medium">생후 100일(3개월) 이하 영아가 38°C 이상</li>
...

  {
    ...
    shortAnswer:
      '타이레놀(아세트아미노펜)과 부루펜(이부프로펜)은 최소 2-3시간 간격으로 교차 복용이 가능합니다.',
    detailedAnswer: `
      <h4 class="font-semibold text-gray-800 mb-2">교차 복용 방법</h4>
      <div class="bg-blue-50 p-4 rounded mb-4">
        <p class="font-medium mb-2">✅ 가능한 조합:</p>
        <p class="text-sm">타이레놀 → (2-3시간 후) → 부루펜 → (2-3시간 후) → 타이레놀</p>
      </div>
      ...
      <div class="bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded">
        <p class="text-sm"><strong>⚠️ 주의</strong></p>
        <p class="text-sm mt-1">교차 복용 시에도 각 해열제의 하루 최대 횟수를 반드시 지켜야 합니다.</p>
      </div>
    `,
    medicalDisclaimer:
      '본 정보는 일반적인 참고 자료입니다. 교차 복용이 필요한 경우 반드시 의사 또는 약사와 상담하세요.',
    sources: [
      {
        name: '해열제정보 (본 사이트)',
        url: '/docs/해열제정보.md',
        type: 'reference',
        description: '교차복용 가능 (최소 3시간 간격)',
      },
      {
        name: 'Fever Coach',
        url: 'https://home.fevercoach.net',
